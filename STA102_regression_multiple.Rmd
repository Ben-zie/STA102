...
$$SCT=\Sigma_{i=1}^{n}(y_{i}-\bar{y})^{2}=||y-\bar{y}||$$
$$SCR=\Sigma_{i=1}^{n}(y_{i}-\hat{y}_{i})^{2}=||y-\hat{y}||=||y-X\beta ||$$
$$SCE=\Sigma_{i=1}^{n}(\hat{y}_{i}-\bar{y})^{2}=||\hat{y}-\bar{y}||$$
...
Donc : 
$$\Rightarrow \ \hat{\sigma}^{2}_{b}=\hat{\sigma}^{2}(X^{T}X)^{-1}$$
$$\Rightarrow \ \hat{\sigma}^{2}_{b_{j}}=\hat{\sigma}^{2}[\ (X^{T}X)^{-1}\ ]_{(j+1,j+1)}$$

Rappel : $b=Ay$      
**$B$ est donc lui aussi un vecteur gaussien, comme transformée linéaire d'un autre vecteur gaussien.**
...
$$\bar{y}=\frac{11^{T}}{n}y$$
On a : $Rang\left(\ \frac{11^{T}}{n} \right)=trace\left(\ \frac{11^{T}}{n} \right)=\Sigma^{n}_{i=1}\frac{1}{n}=1$
...
Alors $SSi \ \ \{\forall i \in p : \beta_{i}=0\}$ :
$$SCT=\Sigma^{n}_{i=1}(y_{i}-\bar{y})^{2} \sim \sigma^{2} \ \chi^{2}_{n-1}$$

car $\{y_{1},...,y_{n}\}$ est une suite de variables normales $iid$ (théorème de Cochrane.)

On sait également maintenant que $SCR \sim \chi^{2}_{(n-p-1)}$.

En appliquant le théorème de Cochrane au triangle $(y\hat{y}\bar{y})$, on obtient que : 
$$SCE=y^{T}\left(H-\frac{11^{T}}{n}\right)y \sim \sigma^{2}\chi^{2}_{p}$$
$$SCE \perp SCR$$
Cela justifie le teste de significativité globale du modèle.

## Tests d'hypothèses sur le modèle :
...
Critères de sélectiond des modèles : 
